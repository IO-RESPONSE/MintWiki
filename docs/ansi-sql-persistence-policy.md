...
| `SERIAL` / `BIGSERIAL` / `gen_random_uuid()` / `uuid_generate_v4()` | 시퀀스/네이티브 UUID 생성 함수는 DB마다 이름과 동작이 다르다 | 애플리케이션 계층에서 문자열 ID(UUID 문자열)를 생성해 저장 (0444 portable ID 정책, `String(255)` PK — [persistence-boundaries.md](persistence-boundaries.md) 참고) |
