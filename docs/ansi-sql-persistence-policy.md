...
- 애플리케이션 연결 문자열/드라이버 선택(`database_url`,
  ...
  잡의 범위다. 0470에서 [config.py](../src/app/config.py)에 `mariadb_dsn`
  필드를 placeholder로 추가했으나, `database.py`는 아직 이 값을 읽지 않고
  기존 PostgreSQL 연결 경로만 쓴다 — 실제 드라이버 전환/이중화는 후속 잡의
  범위로 남는다.
...
| `ILIKE` | MariaDB에는 대소문자 무시 전용 연산자가 없다 | [정규화된 소문자 컬럼](persistence-boundaries.md) + `LIKE`, 또는 collation 기반 대소문자 무시 비교([portable-text-collation-policy.md](portable-text-collation-policy.md) 참고) |
...
- 표준 타입: `VARCHAR(n)`, `TEXT`, `INTEGER`, `BIGINT`, `BOOLEAN`,
  ...
  의 `DateTime(tz)` 규칙과 [0445 portable timestamp 정책](portable-timestamp-column-policy.md)을 따른다).
