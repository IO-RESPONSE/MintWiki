...
SQL)에서, 폴링/락 메커니즘은
[0516](php-db-ui-micro-job-prompts-0351-0670.md)(jobs polling portable
...
- 폴링/락 메커니즘의 구체 SQL(`SELECT ... FOR UPDATE`, `SKIP LOCKED` 대안
  등) — [0516](php-db-ui-micro-job-prompts-0351-0670.md)의 범위다(§7).
...
- `locked_at`/`locked_by`처럼 폴링 클레임(claim)에 필요할 수 있는 컬럼은
  ...
  [0516](php-db-ui-micro-job-prompts-0351-0670.md)에 미루는 질문이라,
...
## 7. 락/폴링 메커니즘은 확정하지 않는다

**핵심 설계: 이 문서는 queued runner가 대기 중인 job을 어떻게
클레임(claim)하는지 확정하지 않는다.** [0516 태스크
노트](php-db-ui-micro-job-prompts-0351-0670.md)("SKIP LOCKED 같은 전용
기능을 기본에서 제외한다")가 이미 이 질문을 명시적으로
[0516](php-db-ui-micro-job-prompts-0351-0670.md)로 미뤄 두었다 —
...
- §3이 `locked_at`/`locked_by` 컬럼을 확정하지 않은 이유가 이것이다 — 락
  ...
  정해진 뒤에야 확정할 수 있다.
- 이 문서가 지금 보장하는 것은 §4의 `status` 값 집합과 §3의
  `available_at` 컬럼뿐이다 — 이 둘만으로도 "폴링 쿼리는 `status='queued'
  AND available_at <= now`로 후보를 좁힌다"는 전제는 이미 성립하며,
  [0516](php-db-ui-micro-job-prompts-0351-0670.md)은 그 후보 집합에서
  동시성 안전하게 하나를 골라내는 방법만 추가로 정하면 된다.
...
- **0468**([jobs table portable SQL](php-db-ui-micro-job-prompts-0351-0670.md)):
  ...
  다시 판단한다.
- **0516**([jobs polling portable policy](php-db-ui-micro-job-prompts-0351-0670.md)):
  §7이 미룬 락/폴링 메커니즘(`SKIP LOCKED` 대안 포함)을 확정하고, §3이
  비워 둔 `locked_at`/`locked_by`류 컬럼 필요 여부를 다시 판단한다.
