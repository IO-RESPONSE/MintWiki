...
/**
 * `public/index.php`가 태스크 0699에서 등록하는 `GET /admin/reports` route의
 * 동작을 확인하는 smoke test. phpunit 없이 `php` CLI만으로 실행된다(0698
 * AdminAuditRouteTest.php와 동일한 방식) — index.php는 재사용 가능한 모듈이
 * 아니므로, 동일한 등록 로직을 Router에 그대로 재구성해 검증한다.
 *
 * 검증 대상:
 * (1) 비로그인(익명) 사용자는 GET /admin/reports에서 `/login`으로 302를 받는다.
 * (2) 로그인했지만 관리자 권한이 없는 사용자는 403(PermissionDeniedPage)을 받는다.
 * (3) 관리자는 200으로 AdminReportListPage를 받고, 스킨 `Layout`의
 *     header/footer를 포함해야 한다.
 * (4) `$accountRepository`가 없으면(DB 미설정/오류) GET /admin/reports는 항상
...
/**
 * `public/index.php`가 등록하는 `GET /admin/reports` 핸들러와 동일한 등록
 * 로직을 재구성한다(위 파일 docblock 참고).
 */
function mintwiki_register_admin_reports_route(
    Router $router,
    ?AccountRepository $accountRepository,
    PhpSessionAdapter $sessionAdapter,
    AclService $aclService
): void {
    $router->register('GET', '/admin/reports', static function () use ($accountRepository, $sessionAdapter, $aclService): Response {
        $layout = new Layout();
...
try {
    ...
    $router = new Router();
    mintwiki_register_admin_reports_route($router, $accountRepository, $sessionAdapter, $aclService);

    // (1) 비로그인(익명) → /login으로 302.
    $_SESSION = [];
    $anonResponse = $router->match(new Request('GET', '/admin/reports'))();
    if ($anonResponse->status() !== 302 || ($anonResponse->headers()['Location'] ?? null) !== '/login') {
        $failures[] = '(1) 비로그인 사용자의 GET /admin/reports는 /login으로 302여야 하는데 status=' . $anonResponse->status()
            . ', Location=' . ($anonResponse->headers()['Location'] ?? '(none)') . '이었다.';
    }

    // (2) 로그인했으나 관리자가 아님 → 403.
    $_SESSION = [SessionUserResolver::SESSION_KEY => $regularId];
    $regularResponse = $router->match(new Request('GET', '/admin/reports'))();
    if ($regularResponse->status() !== 403) {
        $failures[] = '(2) 비관리자 사용자의 GET /admin/reports는 403이어야 하는데 ' . $regularResponse->status() . '이었다.';
    }
    if (!str_contains($regularResponse->body(), '권한 없음')) {
        $failures[] = '(2) 비관리자 거부 응답은 PermissionDeniedPage를 보여줘야 한다.';
    }

    // (3) 관리자 → 200으로 신고 목록 렌더.
    $_SESSION = [SessionUserResolver::SESSION_KEY => $adminId];
    $adminResponse = $router->match(new Request('GET', '/admin/reports'))();
    if ($adminResponse->status() !== 200) {
        $failures[] = '(3) 관리자의 GET /admin/reports는 200이어야 하는데 ' . $adminResponse->status() . '이었다.';
    }

    $adminBody = $adminResponse->body();
    if (!str_contains($adminBody, '<h1>신고</h1>')) {
        $failures[] = '(3) 신고 목록 page가 h1으로 "신고"를 표시해야 한다.';
    }
    if (!str_contains($adminBody, '<!doctype html>')) {
        $failures[] = '(3) 신고 목록 응답이 스킨 Layout의 doctype을 포함해야 한다.';
    }
    if (!str_contains($adminBody, '<header>')) {
        $failures[] = '(3) 신고 목록 응답이 스킨 Layout의 header를 포함해야 한다.';
    }
    if (!str_contains($adminBody, '<footer>')) {
        $failures[] = '(3) 신고 목록 응답이 스킨 Layout의 footer를 포함해야 한다.';
    }
} catch (Exception $e) {
    $failures[] = '(1)-(3) in-process 테스트 실패: ' . $e->getMessage();
}

// (4) $accountRepository가 없으면(DB 미설정) GET /admin/reports는 항상 /login으로 302해야 한다.
try {
    $_SESSION = [SessionUserResolver::SESSION_KEY => 'irrelevant-account-id'];
    $unconfiguredRouter = new Router();
    mintwiki_register_admin_reports_route($unconfiguredRouter, null, new PhpSessionAdapter(), new AclService());

    $unconfiguredResponse = $unconfiguredRouter->match(new Request('GET', '/admin/reports'))();
    if ($unconfiguredResponse->status() !== 302 || ($unconfiguredResponse->headers()['Location'] ?? null) !== '/login') {
        $failures[] = '(4) DB 미설정 상태의 GET /admin/reports는 /login으로 302여야 하는데 status=' . $unconfiguredResponse->status()
...
if ($failures !== []) {
    fwrite(STDERR, "GET /admin/reports route 테스트 실패:\n");
...
fwrite(STDOUT, "GET /admin/reports route 테스트 통과.\n");
exit(0);
