...
if (!str_contains($html, '<form method="post" action="/install/database">')) {
...
if (!str_contains($html, '<label for="host">호스트</label>')) {
    $failures[] = 'DB form이 host 레이블을 포함해야 한다.';
}

if (!str_contains($html, '<input type="text" id="host" name="host"')) {
    $failures[] = 'DB form이 host 텍스트 입력 필드를 포함해야 한다.';
}

if (!str_contains($html, 'placeholder="localhost"')) {
    $failures[] = 'DB form의 host 필드가 placeholder를 가져야 한다.';
}

if (!str_contains($html, '<label for="port">포트</label>')) {
    $failures[] = 'DB form이 port 레이블을 포함해야 한다.';
}

if (!str_contains($html, '<input type="text" id="port" name="port"')) {
    $failures[] = 'DB form이 port 텍스트 입력 필드를 포함해야 한다.';
}

if (!str_contains($html, '<label for="dbname">데이터베이스명</label>')) {
    $failures[] = 'DB form이 dbname 레이블을 포함해야 한다.';
}

if (!str_contains($html, '<input type="text" id="dbname" name="dbname"')) {
    $failures[] = 'DB form이 dbname 텍스트 입력 필드를 포함해야 한다.';
}
...
// (2) 폼 오류가 있는 경우 렌더링
$errors = ['host' => '호스트는 필수입니다.', 'password' => '비밀번호는 필수입니다.'];
...
if (!str_contains($htmlWithErrors, '호스트는 필수입니다.')) {
    $failures[] = '폼 오류가 있을 때 host 오류 메시지를 표시해야 한다.';
}
...
$errors2 = ['host' => 'HOST<script>alert(1)</script>'];
$htmlEscaped = $page2->render($errors2);

if (str_contains($htmlEscaped, '<script>')) {
    $failures[] = 'host 오류 메시지에 포함된 특수 문자가 escaped되지 않았다.';
}
