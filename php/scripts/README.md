...
- `live-http-smoke-test.sh` — 배포된 공개 URL에 대해 실제 `curl` 요청으로
  ...
  실제 HTTP 요청을 보낸다.
- `live-e2e-smoke-test.sh` — 배포된 공개 URL에 대해 관리자/일반 사용자/
  읽기 전용 사용자/익명 사용자 네 세션으로 실제 로그인, 문서 생성/수정/
  삭제(또는 숨김), 사용자 계정 생성, 권한 거부를 curl로 수행하는 API/폼
  E2E smoke test(태스크 0672). `--base-url`이 필수이며, 기존 관리자
  계정은 `SMOKE_ADMIN_USER`/`SMOKE_ADMIN_PASSWORD` 환경변수로 전달한다
  (없으면 installer를 통한 신규 관리자 계정 생성을 먼저 시도한다). 계정
  비밀번호는 커맨드라인 인자나 로그에 남기지 않고 curl config(`-K -`)로만
  전달한다. 각 시나리오는 route가 아직 연결되지 않았거나(404) 계정 생성이
  실패하면 실패로 스크립트를 중단하지 않고 `blocked`/`skip`으로 기록한
  뒤 다음 단계로 넘어간다 — 라이브 배포와 route 연결이 모두 끝나기 전에도
  안전하게 반복 실행할 수 있다. 실행 결과와 현재 알려진 차단 사유는
