...
class InMemorySearchAdapter(SearchAdapter):
    ...
    제목 또는 본문에 질의어가 포함되는지(대소문자 구분 없이) 확인해
    검색 결과를 반환한다. 리다이렉트 검색 등 추가 색인 필드를 활용한
    검색은 후속 태스크에서 다룬다.
    """
    ...
    async def search(self, query: SearchQuery) -> List[SearchResult]:
        """
        제목 또는 본문에 질의어가 포함된 문서를 검색해 결과 목록으로 반환한다.

        Args:
            query: 검색 질의

        Returns:
            질의어가 제목 또는 본문에 포함된 문서의 검색 결과 목록
            (일치하는 문서가 없으면 빈 목록)
        """
        ...
        return [
            ...
            if term in document.title.lower() or term in document.body.lower()
        ]
