...
def require_edit_permission(acl_service: AclService):
    """
    문서 편집(Permission.EDIT) 권한을 요구하는 라우트 의존성을 생성한다.

    require_permission(Permission.EDIT, acl_service) 를 미리 바인딩해 둔
    편의 함수로, 문서 라우터가 편집 보호 라우트를 연결할 때 사용할 자리를
    미리 마련해 둔 골격이다. document_acl/네임스페이스를 요청 경로에서
    읽어 연결하는 작업은 require_permission 과 마찬가지로 이후 태스크에서
    구현한다.
    ...
    Returns:
        FastAPI Depends() 에 전달할 수 있는 의존성 콜러블
    """
    return require_permission(Permission.EDIT, acl_service)
