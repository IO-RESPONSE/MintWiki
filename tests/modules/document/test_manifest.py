"""document 모듈 계약 manifest(`src/modules/document/manifest.json`) 검증.
...
class TestDocumentManifest:
    """document manifest 파일 자체의 형식과 스키마 준수를 검증한다."""
    ...
    def test_manifest_module_name_matches_directory(self):
        """module 필드가 src/modules/document 디렉터리 이름과 일치한다."""
        manifest = _load_manifest()
        assert manifest["module"] == "document"

    def test_manifest_port_source_path_matches_module(self):
        """port.source_path 가 이 모듈 디렉터리를 가리킨다."""
        manifest = _load_manifest()
        assert manifest["port"]["source_path"] == "src/modules/document"
...
class TestDocumentManifestMatchesImplementation:
    ...
    def test_public_methods_exist_on_service(self):
        """manifest 에 선언된 공개 메서드가 DocumentService 에 실제로 존재한다."""
        manifest = _load_manifest()
        for method_name in manifest["service"]["public_methods"]:
            assert hasattr(DocumentService, method_name), (
                f"DocumentService 에 {method_name} 메서드가 없습니다"
            )

    def test_no_undeclared_public_methods(self):
        """DocumentService 의 공개 메서드가 manifest 에 모두 선언되어 있다."""
        manifest = _load_manifest()
        declared = set(manifest["service"]["public_methods"])
        actual = {
            name
            for name, _ in inspect.getmembers(DocumentService, predicate=inspect.isfunction)
    ...
    def test_repository_interface_matches_abstract_base(self):
        """repository.interface 가 DocumentRepository ABC 이름과 일치한다."""
        manifest = _load_manifest()
        assert manifest["repository"]["interface"] == DocumentRepository.__name__
