"""parser 모듈 계약 manifest(`src/modules/parser/manifest.json`) 검증.
...
class TestParserManifest:
    """parser manifest 파일 자체의 형식과 스키마 준수를 검증한다."""
    ...
    def test_manifest_module_name_matches_directory(self):
        """module 필드가 src/modules/parser 디렉터리 이름과 일치한다."""
        manifest = _load_manifest()
        assert manifest["module"] == "parser"

    def test_manifest_port_source_path_matches_module(self):
        """port.source_path 가 이 모듈 디렉터리를 가리킨다."""
        manifest = _load_manifest()
        assert manifest["port"]["source_path"] == "src/modules/parser"
...
class TestParserManifestMatchesImplementation:
    ...
    def test_public_methods_exist_on_service(self):
        """manifest 에 선언된 공개 메서드가 PlainTextBlockParser 에 실제로 존재한다."""
        manifest = _load_manifest()
        for method_name in manifest["service"]["public_methods"]:
            assert hasattr(PlainTextBlockParser, method_name), (
                f"PlainTextBlockParser 에 {method_name} 메서드가 없습니다"
            )

    def test_no_undeclared_public_methods(self):
        """PlainTextBlockParser 의 공개 메서드가 manifest 에 모두 선언되어 있다."""
        manifest = _load_manifest()
        declared = set(manifest["service"]["public_methods"])
        actual = {
            name
            for name, _ in inspect.getmembers(PlainTextBlockParser, predicate=inspect.isfunction)
    ...
    def test_repository_interface_matches_parser_entry_point(self):
        """parser 는 저장소가 없으므로 repository.interface 는 유일한 진입 계약인
        PlainTextBlockParser 이름과 일치해야 한다."""
        manifest = _load_manifest()
        assert manifest["repository"]["interface"] == PlainTextBlockParser.__name__

    def test_parse_returns_parser_result_contract(self):
        """parse(source) 는 blocks/metadata 를 가진 ParserResult 계약을 지킨다."""
