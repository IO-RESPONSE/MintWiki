...
class TestRequireEditPermissionDeniesBlockedUser:
    """차단된 로그인 사용자는 ACL 허용 규칙이 있어도 편집이 거부되는지 확인한다."""
    ...
    def test_blocked_user_is_denied_even_when_acl_allows(self):
        ...
        response = client.get("/document", headers={"X-User-Id": "user-1"})

        assert response.status_code == 403
        assert "detail" in response.json()


class TestRequireEditPermissionAllowsUnblockedUser:
    """차단 서비스가 연결되어 있어도 차단되지 않은 사용자는 편집이 허용되는지 확인한다."""
