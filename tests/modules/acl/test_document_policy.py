"""문서 단위 편집/읽기/토론/이동/삭제/관리자 제한 정책(restrict_document_edit, restrict_document_read,
restrict_document_discuss, restrict_document_move, restrict_document_delete,
restrict_document_admin) 테스트."""
...
class TestRestrictDocumentAdmin:
    """restrict_document_admin이 지정한 대상에게만 관리자 규칙을 부여하는지 확인한다."""

    def test_returns_document_acl_with_single_admin_rule(self):
        acl = restrict_document_admin(
            document_id="doc-1",
            subject_type=SubjectType.GROUP,
            subject_id="admins",
        )
        ...
        assert len(rules) == 1
        rule = rules[0]
        assert rule.id == DOCUMENT_ADMIN_RESTRICTION_RULE_ID
        assert rule.permission is Permission.ADMIN
        assert rule.subject_type is SubjectType.GROUP
        assert rule.subject_id == "admins"
        assert rule.is_allow() is True


class TestRestrictDocumentAdminWithAclService:
    """제한된 문서 ACL이 AclService 검사에서 실제로 관리자 권한을 제한하는지 확인한다."""

    def test_allows_the_designated_group_to_administer(self):
        service = AclService()
        acl = restrict_document_admin(
            document_id="doc-1",
            subject_type=SubjectType.GROUP,
        ...
        assert decision.is_allowed() is True
        assert decision.matched_rule_id == DOCUMENT_ADMIN_RESTRICTION_RULE_ID

    def test_denies_a_different_group_from_administering(self):
        service = AclService()
        acl = restrict_document_admin(
            document_id="doc-1",
            subject_type=SubjectType.GROUP,
    ...
    def test_denies_anonymous_users_from_administering(self):
        service = AclService()
        acl = restrict_document_admin(
            document_id="doc-1",
            subject_type=SubjectType.GROUP,
    ...
    def test_admin_is_denied_by_default_without_document_acl(self):
        service = AclService(namespace_defaults=build_default_namespace_acl_defaults())
