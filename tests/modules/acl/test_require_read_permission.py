"""require_read_permission 편의 의존성 골격 테스트."""
...
from modules.acl.router import CURRENT_USER_ID_STATE_KEY, require_read_permission
from modules.acl.service import AclService


def _build_client(acl_service: AclService) -> TestClient:
    """require_read_permission 으로 보호한 테스트용 라우트를 가진 앱을 만든다."""
    app = FastAPI()
    ...
    @app.get("/document")
    async def document(decision=Depends(require_read_permission(acl_service))):
        return {"allowed": decision.is_allowed()}

    return TestClient(app)


class TestRequireReadPermissionDeniesByDefault:
...
class TestRequireReadPermissionAllowsAnonymous:
    """익명 대상 읽기 허용 규칙이 있으면 통과하는지 확인한다."""
...
class TestRequireReadPermissionIgnoresOtherPermissions:
    """읽기 이외의 권한 규칙만 있으면 거부되는지 확인한다."""

    def test_edit_only_rule_does_not_grant_read(self):
        defaults = NamespaceAclDefaults()
        defaults.register(
            DEFAULT_NAMESPACE,
            [
