...
def test_repository_port_contracts_doc_scopes_out_unimplemented_modules():
    """아직 repository.py 가 없는 admin/audit 모듈은 문서에 새 섹션으로 실리지 않는다."""
    content = DOC_PATH.read_text(encoding="utf-8")
    for module_name in ["admin", "audit"]:
    ...
    assert re.search(r"^## audit", content, re.MULTILINE) is None
