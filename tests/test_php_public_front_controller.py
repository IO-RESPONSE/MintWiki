...
읽고 placeholder 응답만 반환한다")을 지키는지 확인한다.

라우팅이나 애플리케이션 로직은 아직 연결되어 있지 않다 — PHP 내장
웹서버(`php -S`)로 실제 HTTP 요청을 보내 `REQUEST_METHOD`/`REQUEST_URI`
가 응답 본문에 반영되는지, 그리고 CLI로 직접 실행해도 크래시 없이
placeholder 응답을 내는지만 검증한다.
"""
import socket
import subprocess
import time
